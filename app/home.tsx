...
import { requestCampusLocation } from "../utils/locationInput";
...
export default function HomeScreen() {
  ...
  const sortedLocations = [...locations].sort((a, b) => {
    if (selectedFilter === "Menor espera") {
      return a.waitTime - b.waitTime;
    }

    if (selectedFilter === "Dados recentes") {
      if (a.lastReport === "agora" && b.lastReport !== "agora") return -1;
      if (a.lastReport !== "agora" && b.lastReport === "agora") return 1;
      return 0;
    }

    // No protótipo, "Mais perto" usa localização real apenas como input,
    // mas mantém a ordem controlada/simulada dos locais.
    return 0;
  });

  async function handleFilterPress(filter: string) {
    logInteraction("select_home_filter", "home", { filter });

    if (filter === "Mais perto") {
      const result = await requestCampusLocation();

      if (result.granted) {
        Alert.alert(
          "Localização obtida",
          "A localização foi recebida. Neste protótipo, é usada apenas para simular a ordenação por proximidade no campus."
        );
      } else {
        Alert.alert(
          "Localização indisponível",
          "Não foi possível obter a localização. O protótipo continua a usar dados simulados."
        );
      }
    }
        ...
        <ScrollView contentContainerStyle={styles.content}>
          ...
          <View style={styles.cards}>
            {sortedLocations.map((location) => (
              <Pressable
                ...
                style={styles.card}
                onPress={() => {
                  logInteraction("open_location_detail", "home", {
              ...
              >
                <View>
                  ...
                  <Text style={styles.lastReport}>
                    Último reporte {location.lastReport}
                  </Text>
                </View>
          ...
          <View style={styles.actionRow}>
            <Pressable
              ...
              onPress={() => {
                logInteraction("start_report", "home", {
                  source: "main_button",
                });
                router.push("/report");
              }}
            ...
            <Pressable
              ...
              onPress={() => {
                logInteraction("start_alert", "home", {
                  source: "main_button",
                });
