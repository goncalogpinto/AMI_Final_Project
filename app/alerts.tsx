    ...
    <View style={styles.page}>
      <View style={styles.phone}>
        <ScrollView contentContainerStyle={styles.content}>
          ...
          <Text style={styles.subtitle}>Consulta os alertas ativos, alertas recebidos e notificações.</Text>
          ...
          <View style={styles.section}>
            ...
            {activeAlerts.length === 0 && (
              <View style={styles.emptyBox}>
                ...
                <Text style={styles.emptyText}>Ativa um alerta para seres avisado quando uma fila baixar do limite escolhido.</Text>
              </View>
            )}
            ...
            {activeAlerts.map((alert) => (
              <View key={alert.id} style={styles.card}>
                <View style={styles.cardContent}>
                  ...
                  <Text style={styles.cardText}>Avisar abaixo de {alert.threshold} min</Text>
          ...
          <View style={styles.section}>
            ...
            {pastAlerts.length === 0 && (
              <View style={styles.emptyBox}>
                ...
                <Text style={styles.emptyText}>Quando um alerta for atingido, deixa de estar ativo e aparece aqui.</Text>
              </View>
            )}

            {pastAlerts.map((alert) => (
              <View key={alert.id} style={styles.cardPast}>
                <View style={styles.cardContent}>
                  <Text style={styles.cardTitle}>{alert.locationName}</Text>
                  <Text style={styles.cardText}>Limite atingido: abaixo de {alert.threshold} min</Text>
                  <Text style={styles.cardSmall}>Recebido: {alert.triggeredAt ?? "agora"}</Text>
                </View>
          ...
          <View style={styles.section}>
            ...
            {notifications.length === 0 && (
              <View style={styles.emptyBox}>
                ...
                <Text style={styles.emptyText}>As notificações aparecem aqui quando uma fila fica abaixo do limite definido.</Text>
              </View>
            )}
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 15, color: "#627D98", marginTop: 6, marginBottom: 18, lineHeight: 22 },
  primaryButton: { backgroundColor: "#0E7490", borderRadius: 16, paddingVertical: 16, alignItems: "center", marginBottom: 18 },
  ...
  emptyBox: { backgroundColor: "#F6F8FA", borderRadius: 16, padding: 18 },
  emptyTitle: { fontSize: 16, fontWeight: "800", color: "#102A43", marginBottom: 6 },
  emptyText: { fontSize: 14, color: "#627D98", lineHeight: 20 },
  card: { backgroundColor: "#F6F8FA", borderRadius: 16, padding: 16, marginBottom: 12, flexDirection: "row", justifyContent: "space-between", gap: 12 },
  cardPast: { backgroundColor: "#F0FDF4", borderRadius: 16, padding: 16, marginBottom: 12, flexDirection: "row", justifyContent: "space-between", gap: 12 },
  cardContent: { flex: 1 },
  cardTitle: { fontSize: 16, fontWeight: "800", color: "#102A43" },
  cardText: { fontSize: 14, color: "#486581", marginTop: 4 },
  cardSmall: { fontSize: 12, color: "#829AB1", marginTop: 6 },
  cardActions: { alignItems: "flex-end", gap: 8 },
  badge: { alignSelf: "flex-start", paddingHorizontal: 10, paddingVertical: 6, borderRadius: 12, fontSize: 12, fontWeight: "800", overflow: "hidden" },
  badgeActive: { backgroundColor: "#E0F7FA", color: "#0E7490" },
  badgeTriggered: { backgroundColor: "#DCFCE7", color: "#166534" },
  cancelButton: { backgroundColor: "#FEE2E2", borderRadius: 12, paddingHorizontal: 10, paddingVertical: 7 },
  cancelButtonText: { color: "#B91C1C", fontSize: 12, fontWeight: "800" },
  notificationCard: { backgroundColor: "#EFF6FF", borderRadius: 16, padding: 16, marginBottom: 12 },
  notificationTitle: { fontSize: 16, fontWeight: "800", color: "#102A43" },
  notificationText: { fontSize: 14, color: "#486581", lineHeight: 20, marginTop: 6 },
